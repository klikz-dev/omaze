...
import { ISweepstakes , SweepstakesCard } from './SweepstakesCard';
...
describe('SweepstakesCard Component', (): void => {
    ...
    it('should render placeholder if image src is missing', (): void => {
        ...
        expect(image).toHaveClass('bg-black-10');
    });
    ...
    describe('Google Analytics', (): void => {
        beforeEach((): void => {
            render(<SweepstakesCard sweepstakes={mockSweepstakes} />);
        });

        it('does not throw an error if dataLayer does not exist', (): void => {
            expect((): void => {
                const cardLink: HTMLElement = screen.getByRole('link');
        ...
        it('adds event when clicking on card', (): void => {
            window.dataLayer = [];
