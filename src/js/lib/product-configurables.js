 ...
 */
SDG.Product.configurables = function(opts) {
    ...
    function selectCallback(variant, selector) {
        ...
        // update photos
        /* eslint-disable */
        if (typeof ProductSwatches === 'function') {
            ProductSwatches().update(variant);
        }
        /* eslint-enable */
    }
    ...
    function onButtonClick() {
        ...
        if (_.hasClass(this, c.dom.btn_color)) {
            SDG.slider().changeToColor(this);
        }
    }
     ...
     * @param {Object} obj
     */
    function handleVariantOptions(obj) {
        const str = 'option';
        let i;
        let opt;
        let value;
        ...
        for (i = 1; i < 4; i += 1) {
            ...
            if (obj[opt]) {
                value = obj[opt];
                $variant = $form.querySelector(`${c.dom.variant}[data-value="${value}"]`);
    ...
    function selectFirst() {
        ...
        Object.keys($allGroups).forEach((key) => {
            ...
            $prevActive = $group.querySelector(`.${c.cls.active}`);
    ...
    function isDefaultOption() {
        const lowercaseOptions = _.arrayToLower(productData.options);
        let isDefault = false;
...
export default SDG.Product.configurables;