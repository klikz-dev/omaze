...
export function SweepstakesTag (props: ISweepstakesTagProps): ReactElement | null {
    ...
    const tagClassName: string = `oz-type-subtitle1 oz-px-1 oz-py-1/2 oz-rounded ${className}`;
    ...
    if (days > 2) {
        return <div className={`${tagClassName} oz-bg-rebrand-yellow-500`}>{days} days left!</div>;
    }

    if (hours > 0) {
        return <div className={`${tagClassName} oz-bg-rebrand-yellow-500`}>{hours} hours left!</div>;
    }

    if (hasWinner) {
        return <div className={`${tagClassName} oz-bg-white`}>See who won!</div>;
    }
